...
#define SSD1306_SELECT gpio_put(csPin, 0)            ///< Device select
#define SSD1306_DESELECT gpio_put(csPin, 1)          ///< Device deselect
#define SSD1306_MODE_COMMAND gpio_put(dcPin, 0)      ///< Command mode
#define SSD1306_MODE_DATA gpio_put(dcPin, 1)         ///< Data mode

#define SETWIRECLOCK i2c_init(wire, clkDuring) ///< Set before I2C transfer
#define RESWIRECLOCK i2c_init(wire, clkAfter) ///< Restore after I2C xfer
// #endif
...
/*!
    ...
    @param  wire
            Pointer to pico's wire instance (e.g. &Wire, the
...
*/
  ...
  Adafruit_SSD1306::Adafruit_SSD1306(uint8_t w, uint8_t h, i2c_inst_t *wire, int8_t rst_pin, uint clkDuring, uint clkAfter)
  : Adafruit_GFX(w, h), spi(NULL), wire(wire), buffer(NULL), mosiPin(-1), clkPin(-1), dcPin(-1), csPin(-1), rstPin(rst_pin)

{
  ...
  _cp437 = false;
  gfxFont = NULL;
  this->clkDuring = clkDuring;
  this->clkAfter = clkAfter;
}
...
void Adafruit_SSD1306::ssd1306_commandList(const uint8_t *c, uint8_t n) {
  if (wire)
  {
    ...
    buf[0] = 0x00;
    memcpy(buf+1, c, n);
    i2c_write_blocking(wire, i2caddr, buf, n+1, false);
...
bool Adafruit_SSD1306::begin(uint8_t vcs, uint8_t addr, bool reset, bool periphBegin) {
  ...
  clearDisplay(); //just clears itnernal buffer
...
void Adafruit_SSD1306::display(void) {
  ...
  // }
  uint8_t buf[count+1];
  buf[0] = 0x40; // first byte sent is the command byte
  memcpy(buf+1, ptr, count); // copy the rest of the data in ptr into buf[1] onwards (so as not to overwrite command buf[0])
  i2c_write_blocking(wire, i2caddr, buf, count+1, false); // do the deed! >:)
  TRANSACTION_END
}
